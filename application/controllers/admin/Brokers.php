...
//    }
    ...
    function agentbrokesJson() {
        ...
        $aColumns = array("user_id", "display_name", "email", "created_date");
        ...
        $sWhere = "";
        $sColumns = array("display_name", "email", "created_date");
        if ($_GET['sSearch'] != "") {
            $sWhere .= " (";
    ...
    function indexJson() {
        ...
        if ($rResult) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] .= '<a class="btn btn-info waves-effect waves-light btn-xs view_profile" href="' . base_url() . 'admin/brokers/profile?user_id=' . urlencode(base64_encode($aRow["user_id"])) . '" data-target="#tabs-modal" data-toggle="modal"><span class="btn-label"><i class="fa fa-eye"></i></span>View Profile </a> &nbsp'
                        . '<button type="button" class="btn btn-success waves-effect waves-light btn-xs approved" value="' . urlencode(base64_encode($aRow['user_id'])) . '"><span class="btn-label"><i class="fa fa-check"></i></span>Approved</button> &nbsp'
                        . '<button type="button" class="danger-alert btn btn-danger waves-effect waves-light btn-xs disapproved" value="' . urlencode(base64_encode($aRow['user_id'])) . '"><span class="btn-label"><i class="fa fa-times"></i></span>Disapproved</button>';
