...
.select-product-model .modal-body .row{border-bottom:2px solid rgba(255, 255, 255, 0.07);padding: 15px 0px;margin: 0 30px;}    .select-product-model .modal-body .row:last-child{border-bottom:none;}    
...
.select-product-model .modal-body button.btn.btn-danger span.btn-label {margin-right: 2px;}
    ...
    <div class="modal-dialog modal-full select-product-model">    
    	<div class="modal-content">        
     	...
     	<input type="hidden" value="<?php echo $member_id; ?>" id="memberid">        
     		...
     		<?php } ?>                        
     			<div class ="col-md-3 col-sm-6 col-xs-12 text-center">
                    <div class="site-image">
                       	<img src="<?php echo site_url() . '/assets/crm_image/products/' . $product['product_image']; ?>">
                    </div>                            
                	<div class="pro_info"> 
                    ...
                    <div class="pro_info"> 
	                    <label>Product Coverage: </label>
	                     <span><?php echo $product['product_coverage']; ?></span>
                    </div>                            
                    <div class="pro_info"> 
		                 <label>Plan ID: </label> 
		                 <span><?php echo $product['plan_id']; ?></span>
                    </div>                            
                    <div class="button-list pro_info">                                
	                     <button type="button" class="btn btn-success waves-effect waves-light product-add pro-add-<?= $key ?>" data-id="<?= $key ?>" data-custom-value-productid="<?php echo $product['global_product_id']; ?>" >                                    
	                     <span class="btn-label"><i class="fa fa-plus"></i>                                    
	                     </span>Add</button>                                
	                     <button type="button" class="btn btn-danger waves-effect waves-light product-remove pro-remove-<?= $key ?>" data-id="<?= $key ?>" data-custom-value-productid="<?php echo $product['global_product_id']; ?>" disabled>                                    
