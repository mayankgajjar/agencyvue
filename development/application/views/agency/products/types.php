            ...
            <div class="col-lg-12">
                <div class="card-box">
                    <div class="row">
                        ...
                        <div class="col-sm-2">
                            <button class="btn btn-primary waves-effect waves-light" data-toggle="modal" data-target="#custom-width-modal">Add Product Type</button>
                        </div>
                    </div>
    ...
    <div class="modal-dialog" style="width:35%;">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="custom-width-modalLabel">Add New Product Type</h4>
            </div>
            <div class="modal-body">
                <form id="frm_add_pcate" method="POST">
                    <div class="form-group">
                        <label for="product_categories">Product Category Name*</label>
                        <select class="form-control select2" name="cate" id="cate-select" data-placeholder="Select Vendor">
                            <option value="">Select Product Category </option>
                            <?php foreach ($product_categoies as $category): ?>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="name" placeholder="NAME OF Type" name="name">
...
<div id="edit-custom-width-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="custom-width-modalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog" style="width:35%;">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="fa fa-times"></i></button>
                <h4 class="modal-title" id="custom-width-modalLabel">Edit Product Type</h4>
            </div>
            <div class="modal-body">
                <form id="frm_add_pcate" method="POST">
                    ...
                    <div class="form-group">
                        <label for="product_categories">Product Category Name*</label>
                        <select class="form-control select2" name="cate" id="cate-edit" data-placeholder="Select Vendor">
                            <option value="">Select Product Category </option>
                            <?php foreach ($product_categoies as $category): ?>
                                <option value="<?= $category['product_category_id']; ?>"><?= $category['product_category_name']; ?></option>
                            <?php endforeach; ?>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="name-e" placeholder="NAME OF Type" name="edit_name">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#datatable').dataTable({
            ...
            "sAjaxSource": "<?php echo base_url() ?>agency/products/typeJson"
        });
    ...
    $("#add_cate").click(function (e) {
        ...
        if (id == "") {
            $('#error-name').show();
            $("#cate-select").addClass("textInputError");
            error = error + 1;
        }
        ...
        if (error == 0) {
            $.ajax({
                type: "POST",
                url: "<?php base_url() ?>add_edit_type",
                data: {name: name, id: id, method: 'add'},
    ...
    jQuery(document).on('click', '.edit_product_category', function () {
        jQuery("#typeid").val(jQuery(this).data('typeid'));
        jQuery("#name-e").val(jQuery(this).data('typename'));
        jQuery("#cate-edit").val(jQuery(this).data('cateid'));
        jQuery('#edit-custom-width-modal').modal('show');
    });
    ...
    $("#edit_cate").click(function (e) {
        ...
        if (error == 0) {
            $.ajax({
                type: "POST",
                url: "<?php base_url() ?>add_edit_type",
                data: {name: name, id: id, cate: cate_id, method: 'edit'},
                success: function (data) {
                    swal("success", data, "success");
                    var $lmTable = $("#datatable").dataTable({bRetrieve: true});
                    $lmTable.fnDraw();
