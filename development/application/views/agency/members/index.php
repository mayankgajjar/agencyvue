            ...
            <div class="col-lg-12 lead_list">
                <div class="card-box lead-page">
                    <div class="row">
                        ...
                        <div class="col-sm-2">
                            <a class="btn btn-primary waves-effect waves-light" href="<?= base_url() . 'agency/members/add_member' ?>">Add Clients</a>
                        </div>
                        <div class="col-sm-4">
                            <select class="form-control input-sm agent-fliter" id="agent-selector" name="agent-fliter">
                                <option value="">Show All Clients</option>
                                <?php foreach ($agencts as $agenct): ?>
                                    <option value="<?= $agenct['user_id'] ?>"><?= $agenct['display_name'] ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-sm-6">
                            <form role="form">
                                <div class="form-group contact-search m-b-30">
                                    <a class="btn btn-success waves-effect waves-light" href="<?= base_url() . 'agency/settings/Bluk_Member_Upload' ?>">Bulk Client Upload</a>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#datatable').dataTable({
            ...
            "sAjaxSource": "<?php echo base_url() ?>agency/members/membersJson"
        });
        ...
        $(document).on("click", ".del_lead", function () {
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        method: "POST",
                        url: '<?php echo base_url() ?>agency/members/removemember?>',
        ...
        $(document).on("click", ".active-member", function () {
            ...
            $.ajax({
                method: "POST",
                url: '<?php echo base_url() ?>agency/members/activemember',
                data: {user_id: user_id},
                success: function (data) {
                    swal("success", data, "success");
                    var $lmTable = $("#datatable").dataTable({bRetrieve: true});
        ...
        $(document).on("click", ".inactive-btn", function () {
            ...
            if (selectReason == "") {
                ...
                $.ajax({
                    method: "POST",
                    url: '<?php echo base_url() ?>agency/members/inactivemember',
        ...
        $(document).on('click', '.lead_det', function () {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url() ?>agency/members/show_member_info",
                data: {memberid: id},
                success: function (data) {
