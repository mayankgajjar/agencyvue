...
class Leads extends CI_Controller {
    ...
    public function index() {
        $data['title'] = "Leads Management || Agency";
        $data['agencts'] = get_agents($this->session->userdata['user_info']['user_id']);
        $this->template->load('agency_header', 'agency/lead/index', $data);
    }
     ...
     */
    function leadsJson() {
        $agency_id = $this->session->userdata['user_info']['user_id'];
        $agent_id = "";
        ...
        $aColumns = array("customer_id", "cname", "customer_email", "display_name", "customer_created_date");
        ...
        $sWhere = "";
        if ($_GET['sSearch'] != "") {
            $sColumns = array("customer_first_name", "customer_last_name", "customer_email", "display_name", "customer_created_date");
            $sWhere .= "(";
        ...
        if ($agent_id != "") {
            $rResult = $this->lead->get_leads_agency($sLimit, $sWhere, $sOrder, $agent_id);
            $iTotal_f = count($rResult);
            $iTotal = $this->lead->get_leads_count_agency($sWhere, $agent_id);
        } else {
            $rResult = $this->lead->get_leads_agency($sLimit, $sWhere, $sOrder, rtrim($leadPerents, ","));
            $iTotal_f = count($rResult);
            $iTotal = $this->lead->get_leads_count_agency($sWhere, rtrim($leadPerents, ","));
        }
        $output = array(
        ...
        if ($rResult) {
            foreach ($rResult as $aRow) {
                ...
                $row[] .= '<a class="btn btn-info btn-xs table-action-btn admin-action-icon-in-data-table" href="#"title="View lead Profile"><i class="glyphicon glyphicon-eye-open" title="View Profile"></i></a>'
                        . '<a href="' . base_url() . 'agency/leads/edit_leads/' . urlencode(base64_encode($aRow["customer_id"])) . '" " class="btn btn-default btn-xs table-action-btn admin-action-icon-in-data-table"><i class="fa fa-pencil"></i></a>'
    ...
    /**
     * @uses Remove or delete lead || is_deleted Status
     * @author dHareen
     */
    ...
    public function edit_leads($id = null) {
        ...
        $data['add_child_block_html'] = $this->load->view('agency/lead/get_child_view', $new_block, true);
        ...
        $this->template->load('agency_header', 'agency/lead/edit_lead', $data);
    }
    ...
    function addproduct() {
        ...
        if ($leadStatus['customer_status'] == "lead") {
            ...
            $memberLog = array(
                'action' => 'lead_to_member',
                'customer_id' => $leadID,
                'action_by' => $this->session->userdata['user_info']['user_id']
            );
            $log = insert_update_data($ins = 1, $tbl_name = 'crm_member_log', $ins_data = $memberLog);
