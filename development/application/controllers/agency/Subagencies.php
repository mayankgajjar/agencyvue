...
/**
 * ManageAgency : Controller for managing sub agency in the agency user role.
 * @author dhareen
 */
defined('BASEPATH') OR exit('No direct script access allowed');

class Subagencies extends CI_Controller {
    ...
    public function index() {
        ...
        $this->template->load('agency_header', 'agency/subagency/index', $data);
    }

    /*
     * Data Table JSON CREATER || Searching || Pagination || SORTING
     */

    public function subagencyJson() {

        $agencyId = $this->session->userdata['user_info']['user_id'];
        $agent_id = "";
        ...
        $rResult = $this->agency->get_sub_agencies($sLimit, $sWhere, $sOrder, $agencyId);
        $iTotal_f = count($rResult);
        $iTotal = $this->agency->get_sub_agencies_count($sWhere, $agencyId);
        $output = array(
        ...
        if ($rResult) {
            foreach ($rResult as $aRow) {
                ...
                $row[] .= '<a class="btn btn-info btn-xs table-action-btn admin-action-icon-in-data-table" href="' . base_url() . 'agency/subagencies/view?user_id=' . urlencode(base64_encode($aRow["user_id"])) . '"  title="View Agency Profile"><i class="glyphicon glyphicon-eye-open" title="View Profile"></i></a>'
                        . '<a href="' . base_url() . 'agency/subagencies/edit?user_id=' . urlencode(base64_encode($aRow["user_id"])) . '" " class="btn btn-default btn-xs table-action-btn admin-action-icon-in-data-table" title="Edit Agency Profile"><i class="fa fa-pencil"></i></a>'
    ...
    /*
     * Add New Agency, Create log || Create new user in user_tbl ( USER MASTER )  || Agencies BASIC table || Domain Master
     */

    public function add() {
        $data['title'] = "Add New Sub Agency || AgencyVue";
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($agency_w9['status'] == 1 && $agency_dd['status'] == 1 && $agency_ca['status'] == 1 && $agnecyLogo['status'] == 1) {
                    ...
                    if ($agencyTbl && $domain && $user_tbl && $agencybankTble && $agencydocTbl && $agencylog) {
                        ...
                        redirect('agency/subagencies/add');
                    }
                } else {
                    $msg = "";
        ...
        $this->template->load('agency_header', 'agency/subagency/add', $data);
    }
    ...
    /**
     * Edit Agency Page with server side validation
     */
    public function edit() {
        $data['title'] = "Sub Agency Edit";
        $data['state'] = get_data($tbl_name = 'crm_states', $single = 0);
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if ($agencyTbl) {
                        ...
                        redirect('agency/subagencies/edit?user_id=' . $_REQUEST['user_id']);
                    }
                }
            }
            $this->template->load('agency_header', 'agency/subagency/edit', $data);
        } else {
            exit("Agency ID NULL || ERROR");
        }
    }
    ...
    public function view() {
        ...
        if (isset($_REQUEST['user_id'])) {
            $userId = base64_decode(urldecode($_REQUEST['user_id']));
            $data['details'] = $this->agency->get_agency_details($userId);
            $this->template->load('agency_header', 'agency/subagency/view', $data);
        } else {
            exit("Agency ID NULL || ERROR");
        }
    }
