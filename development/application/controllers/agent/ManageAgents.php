...
class ManageAgents extends CI_Controller {
    ...
    public function index() {
        ...
        $this->template->load('agent_header', 'agent/manage_agents/index', $data);
    }
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($_FILES['img']['name'])) {
                    ...
                    $config['allowed_types'] = 'jpg|jpeg|bmp|gif|png|pdf';
                ...
                if ($user_tbl && $broker_personal && $broker_business && $broker_commision) {
                    $agentAddFeed = array(
                        ...
                        'feed_type' => 'feed',
                        'feed_icon' => 'glyphicon glyphicon-user',
                        'feed_title' => 'New Agent Added In Your Profile || ' . get_display_name($user_id) . ' || Waiting For Admin Approval',
                        'url' => base_url() . 'agent/ManageAgents/brokerEdit?user_id=' . urlencode(base64_encode(get_member_from_userID($user_id)))
                    );
                ...
                redirect(base_url() . 'agent/ManageAgents');
            }
        }

        $this->template->load('agent_header', 'agent/manage_agents/add_agent', $data);
    }
    ...
    function brokerEdit() {
        ...
        if ($this->input->post()) {
            ...
            $user_table = insert_update_data($ins = 0, $table_name = 'crm_user_tbl', $ins_data = $data_crm_user_tbl, $where = $where);
            ...
            redirect(base_url() . 'agent/ManageAgents/brokerEdit?user_id=' . $_REQUEST['user_id']);
        }

        $this->template->load('agent_header', 'agent/manage_agents/manageBrokers.php', $data);
    }
    ...
    public function brokerProfile() {
        $data['downAgency'] = $data['downEmployers'] = $data['downAgents'] = $data['downMembers'] = "";
        ...
        $this->template->load('agent_header', 'agent/manage_agents/broker_profile_view', $data);
    }
