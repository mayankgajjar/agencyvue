...
//        Step 2: Retention Loss - 100% = Retention Rate
        ...
        if ($this->input->post()) {
            ...
            if (!empty($_FILES['logo']['name'])) {
                ...
                if (!$this->upload->do_upload('logo')) {
                    ...
                    //$this->session->userdata['agent_profile_img'];
            ...
            $profileFeed = array(
                ...
                'feed_type' => 'feed',
                'feed_icon' => 'glyphicon glyphicon-saved',
                'feed_title' => 'Profile Updated',
                'url' => ''
            );
            insert_update_data(1, 'crm_feeds', $profileFeed);
    ...
    /**
     * @uses Navigation menu to set Agent's Target || AJAX METHOD
     */
    public function set_target_nav_memu() {
        if ($this->input->post('agent_target') == "") {
            exit('ERROR IN TARGET VALUE || AGENCY VUE');
        }
        $this->db->set('agent_target', $this->input->post('agent_target'));
        $this->db->where('user_id', $this->session->userdata['user_info']['user_id']);
        $this->db->update('crm_broker_personal');
        echo 'Target Updated !';
        die();
    }
