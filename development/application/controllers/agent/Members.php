    ...
    function indexJson() {
        ...
        if ($rResult) {
            ...
            foreach ($rResult as $aRow) {
                ...
                if ($aRow['is_active'] == "Y") {
                    $row[] .= '<a href="' . base_url() . 'agent/members/edit_member/' . urlencode(base64_encode($aRow["customer_id"])) . '" class="edit_btn btn btn-info btn-xs view_profile table-action-btn admin-action-icon-in-data-table edit-href"><i class="glyphicon glyphicon-pencil" title="Edit Client"></i></a>
...
//            }
        ...
        if (($this->input->post('upload'))) {
            ...
            if (!empty($_FILES['verification_scripts']['name'])) {
                ...
                $config['allowed_types'] = 'mp3|wav';
                $config['max_size'] = 5120;
                $config['name'] = $_FILES['verification_scripts']['name'];
                $this->load->library('upload', $config);
                $this->upload->initialize($config);
                if ($this->upload->do_upload('verification_scripts')) {
                        $fileData = $this->upload->data();
                        $data_array = array(
                            'script' => $fileData['file_name'],
                ...
                if ($insert_script) {
                    ...
                    $leadAddFeed = array(
                        ...
                        'feed_title' => 'Verification Script Uploaded In Client Profile',
                    );
                    insert_update_data(1, 'crm_feeds', $leadAddFeed);
                }
            ...
            if (!empty($_FILES['member_file']['name'])) {
                ...
                if ($this->upload->do_upload('member_file')) {
                    ...
                    if ($insert_file) {
                        ...
                        insert_update_data(1, 'crm_feeds', $leadAddFeed);
                        if ($leadAddFeed) {
                            $this->session->set_flashdata('success', 'File Is Added In Client Profile');
                            redirect(base_url('agent/members/edit_member/' . $id));
                        }
                    }
                } else {
                ...
                $this->session->set_flashdata('error', 'Required File ||Upload File In Client File Section');
                redirect(base_url('agent/members/edit_member/' . $id));
            }
        }
                ...
                if ($old_file != $this->input->post('file_name')) {
                    ...
                    if ($rename == 1) {
                        ...
                        if ($updateFileDetails) {
                            ...
                            insert_update_data(1, 'crm_feeds', $leadAddFeed);
                            $this->session->set_flashdata('success', 'File\'s Details Are Update In Client Profile');
                            redirect(base_url('agent/members/edit_member/' . $id));
                        }
                    }
                } else {
    ...
    public function inactivemember() {
        ...
        if ($done) {
            ...
            echo "Error Into InActive Client";
        }
    }
    ...
    function product_detail()
    {
        ...
        $sql = "SELECT * FROM crm_products pro LEFT JOIN crm_vendor_primary vp on pro.product_vendor = vp.vendor_id LEFT JOIN crm_member_products mp on pro.global_product_id = mp.product_id LEFT JOIN crm_product_enrollment_fee pef on pro.global_product_id = pef.global_product_id LEFT JOIN crm_product_commission cpm on pro.global_product_id = cpm.product_id where pro.global_product_id = ".$product_id." AND mp.member_product_id = ".$member_product_id ;
        $data['product'] = $this->db->query($sql)->row();
        echo $this->load->view('agent/member/product_detail', $data, TRUE);
    }
