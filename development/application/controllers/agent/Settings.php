...
class Settings extends CI_Controller {
    ...
    public function blukleadupload() {
        $data['title'] = "Bulk Lead Upload || Agent";
        ...
        $this->template->load('agent_header', 'agent/bulklead/index', $data);
    }

    public function Bluk_Member_Upload() {
        $data['title'] = "Bulk Member Upload || Agent";
        if ($this->input->post("Upload")) {
            $this->load->library('upload');
        ...
        $this->template->load('agent_header', 'agent/bulkmember/index', $data);
    }
    ...
    public function target_setting() {
        if ($this->input->post('agent_target_members')) {
            if ($this->input->post('members_range') == 'daily') {
                $resetTime = date('Y-m-d', strtotime(' +1 day'));
            }
            if ($this->input->post('members_range') == 'weekly') {
                $resetTime = date('Y-m-d', strtotime('next monday', strtotime(date('Y-m-d'))));
            }
            if ($this->input->post('members_range') == 'monthly') {
                $resetTime = date('Y-m-01', strtotime('+1 MONTH'));
            }
            if ($this->input->post('members_range') == 'quarterly') {
                $resetTime = date('Y-m-01', strtotime('+4 MONTH'));
            }
            if ($this->input->post('members_range') == 'yearly') {
                $endDate = new DateTime('1st January Next Year');
                $resetTime = $endDate->format('Y-m-d'); // 2017-01-01
            }
            $memberArray = array('target_type' => 'members',
                'target_range' => $this->input->post('members_range'),
                'target_value' => $this->input->post('agent_target_members'),
                'agent_id' => $this->session->userdata['user_info']['user_id'],
                'reset_time' => $resetTime);
            insert_update_data(1, 'crm_agent_target', $memberArray);
        }

        if ($this->input->post('agent_target_commission')) {
            if ($this->input->post('premium_range') == 'daily') {
                $resetTime = date('Y-m-d', strtotime(' +1 day'));
            }
            if ($this->input->post('commission_range') == 'weekly') {
                $resetTime = date('Y-m-d', strtotime('next monday', strtotime(date('Y-m-d'))));
            }
            if ($this->input->post('commission_range') == 'monthly') {
                $resetTime = date('Y-m-01', strtotime('+1 MONTH'));
            }
            if ($this->input->post('commission_range') == 'quarterly') {
                $resetTime = date('Y-m-01', strtotime('+4 MONTH'));
            }
            if ($this->input->post('commission_range') == 'yearly') {
                $resetTime = date('Y-m-d', strtotime(' +1 day'));
            }
            $commissionArray = array('target_type' => 'commission',
                'target_range' => $this->input->post('commission_range'),
                'target_value' => $this->input->post('agent_target_commission'),
                'agent_id' => $this->session->userdata['user_info']['user_id'],
                'reset_time' => $resetTime);
            insert_update_data(1, 'crm_agent_target', $commissionArray);
        }
        if ($this->input->post('agent_target_premium')) {
            if ($this->input->post('premium_range') == 'daily') {
                $resetTime = date('Y-m-d', strtotime(' +1 day'));
            }
            if ($this->input->post('premium_range') == 'weekly') {
                $resetTime = date('Y-m-d', strtotime('next monday', strtotime(date('Y-m-d'))));
            }
            if ($this->input->post('premium_range') == 'monthly') {
                $resetTime = date('Y-m-01', strtotime('+1 MONTH'));
            }
            if ($this->input->post('premium_range') == 'quarterly') {
                $resetTime = date('Y-m-01', strtotime('+4 MONTH'));
            }
            if ($this->input->post('premium_range') == 'yearly') {
                $resetTime = date('Y-m-d', strtotime(' +1 day'));
            }
            $premiumArray = array('target_type' => 'premium',
                'target_range' => $this->input->post('premium_range'),
                'target_value' => $this->input->post('agent_target_premium'),
                'agent_id' => $this->session->userdata['user_info']['user_id'],
                'reset_time' => $resetTime);
            insert_update_data(1, 'crm_agent_target', $premiumArray);
        }
        echo 'done';
    }
    ...
    public function verification() {
        ...
        //pr_exit($agentMember);
        $this->template->load('agent_header', 'agent/settings/verification', $data);
    }
    ...
    public function verificationJson() {
        ...
        $rResult = $this->member->get_member_script($sLimit, $sWhere, $sOrder);
        $iTotal_f = count($rResult);
        $output = array(
            "sEcho" => intval($_GET['sEcho']),
            "iTotalRecords" => $iTotal_f,
            "iTotalDisplayRecords" => $this->member->get_member_script_count($sWhere),
            "aaData" => array()
        );
        if ($rResult) {
