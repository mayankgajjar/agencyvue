    ...
    public function addProduct() {                
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $productTable = array(
                ...
                'product_price' => $productPrice,
                'product_coverage' => $post['coverage'],
                'product_pre_existing' => $post['allow_conditions'],
                'product_status' => $post['product_status'],
                'product_requires_appointment' => $post['appointment'],
                ...
                'product_requires_license' => $post['license'],
                'product_enrollment_billing_rule' => $post['enrollment_billing'],
                'product_next_billing_date_rule' => $post['next_billing_date_rule'],
                'product_billing_rule' => $post['billing_rule'],
                'product_next_billing_date_rule' => $post['next_billing_date_rule'],
                'product_activation_date_rule' => $post['activation_rule'],
                'product_image' => $product_image,
                'verification_script' => $verification_script_data,
                'product_vendor' => $post['vendor'],
            ...
            $productCommssion = array(
                ...
                'commission_structure' => isset($post['commission_structure']) ? $post['commission_structure'] : '',
                'premium' => isset($post['premium']) ? $post['premium'] : '',
                'premium_duration' => isset($post['premium_duration']) ? $post['premium_duration'] : '',
                'commissionable_premium' => isset($post['commissionable_premium']) ? $post['commissionable_premium'] : '',
                'percent_premium' => isset($post['percent_premium']) ? $post['percent_premium'] : '',
            );

            $productCOM = insert_update_data($ins = 1, $table_name = 'crm_product_commission', $ins_data = $productCommssion);
