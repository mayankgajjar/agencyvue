 ...
 //   var $products = '';
    ...
    public function add_quote() {
        ...
        if ($this->input->post('save')) {
            ...
            $age = round($Age);
            $include_spouse1 = $this->input->post('spouse');
            $gender1 = $this->input->post('gender');
            $spouse_gender1 = $this->input->post('spouse_gender');
            $allow_tobacco1 = $this->input->post('allow_tobacco');
            $state1 = $this->input->post('quote_state');
            $zipcode1 = $this->input->post('zipcode');
            $spouse_dob1 = $this->input->post('spouse_dob');
            $childern_count1 = $this->input->post('childern_count');
            $Age = ((time() - strtotime($spouse_dob1)) / (3600 * 24 * 365));
            $spouse_age = round($Age);
            ...
            $include_spouse = urlencode(base64_encode($this->input->post('spouse')));
            $gender = urlencode(base64_encode($this->input->post('gender')));
            $spouse_gender = urlencode(base64_encode($this->input->post('spouse_gender')));
            $allow_tobacco = urlencode(base64_encode($this->input->post('allow_tobacco')));
            $spouse_age_e = urlencode(base64_encode($spouse_age));
            $childern_count = urlencode(base64_encode($this->input->post('childern_count')));
            if (isset($ful_type)) {
            ...
            redirect(base_url() . 'agent/quote/find_product?state=' . $state_id . '&age=' . $dob . '&zip=' . $zipcode .'&include_spouse='. $include_spouse . '&gender='. $gender . '&allow_tobacco=' . $allow_tobacco . '&spouse_age='. $spouse_age_e . '&spouse_gender='. $spouse_gender .'&childern_count='. $childern_count);
        }
    }

    public function match_api_product($apiProducts, $state_products)
    {
        $count = 0;
        foreach ($apiProducts as $aProduct) {
                if (count($aProduct->Plan) == count($aProduct->Plan, COUNT_RECURSIVE)){
                    $product_name = explode("~~", $aProduct->Plan->Name)[0];
                    $result = match_product($product_name);
                    if (count($result) > 0)
                    {
                        $array_state_id = array_column($state_products, 'global_product_id');
                        if(!in_array($result['global_product_id'], $array_state_id))
                        {
                            $sql = "SELECT enrollment_fee FROM crm_product_enrollment_fee WHERE global_product_id = " . $result['global_product_id'];
                            $enrollment_fee = $this->db->query($sql)->row_array();
                            $result['enrollment_fee'] = $enrollment_fee['enrollment_fee'];
                            $apiProductLocal[$count] = $result;
                        }
                    }
                    else
                    {
                        $apiProductLocal[$count] = $aProduct->Plan;
                    }
                    $count++;
                }
            }
            return $apiProductLocal;
    }
    public function filters() 
    {
            ...
            foreach ($parameters as $key => $value) {
                $key = explode('=',$value)[0];
                $para_value =  base64_decode(urldecode(explode('=',$value)[1]));
                $parameter_array[$key] = $para_value;
            }
            $apiProducts = quoteRequest($parameter_array);
            $data['stateProducts'] = $this->product->findproduct($parameter_array['state'], $parameter_array['age'], $parameter_array['zip']);
            $data['api_products'] = $this->match_api_product($apiProducts, $data['stateProducts']);
           // pr_exit($this->find_product($flag = 1));
            $data['cost'] = $cost;
    ...
    public function find_product() {
        ...
        $this->session->set_userdata('query_string', $query_string );
        $data['title'] = 'Find Product';
        $state_id = base64_decode(urldecode($_REQUEST['state']));
        $age = base64_decode(urldecode($_REQUEST['age']));
        $zip = base64_decode(urldecode($_REQUEST['zip']));
        $include_spouse = base64_decode(urldecode($_REQUEST['include_spouse']));
        $allow_tobacco = base64_decode(urldecode($_REQUEST['allow_tobacco']));
        $gender = base64_decode(urldecode($_REQUEST['gender']));
        $spouse_age = base64_decode(urldecode($_REQUEST['spouse_age']));
        $spouse_gender = base64_decode(urldecode($_REQUEST['spouse_gender']));
        $childern_count = base64_decode(urldecode($_REQUEST['childern_count']));
        $this->load->model('product');
        $apiProductLocal = array();
        $apiProducts = quoteRequest(array('age' => $age, 'zip' => $zip, 'state' => $state_id,'include_spouse' => $include_spouse, 'allow_tobacco' => $allow_tobacco, 'gender'=> $gender, 'spouse_age'=>$spouse_age, 'spouse_gender'=> $spouse_gender, 'childern_count'=> $childern_count));
        ...
        // fwrite($fp, json_encode($data['api_products']));
        // fclose($fp);
        // end of json writing into file
        // $json = file_get_contents('assets/json_temp/1515391731.json');
    ...
    public function member_info($id = null) {
        ...
        $data['title'] = 'Client Information || Admin';
    ...
    function checkout_script($customer_id = null) {
        ...
        foreach ($scripts_data as $key => $script) {
            // echo '<pre>';
            // print_r($script[$key][$key]['script_html']);
            // echo '</pre>';
            array_push($scripts_arr, array('product_name' => get_product_global_id($script[$key]['global_product_id']), 'script_html' => isset($script[$key]['script_html']) ? $script[$key]['script_html'] : $script[$key][$key]['script_html'], 'member_id' => base64_decode(urldecode($customer_id)), 'customer_name' => $member_details['customer_first_name'] . ' ' . $member_details['customer_last_name'], 'customer_address' => $member_details['customer_address'], 'customer_address_details' => $member_details['customer_address_details'], 'customer_city' => $member_details['customer_city'], 'customer_state' => $member_details['customer_state'], 'customer_zipcode' => $member_details['customer_zipcode']));
        }
    ...
    public function complete($customer_id = NULL) {
        ...
        if (($this->input->post('checkout'))) {
            ...
            if (!empty($_FILES['verification_scripts']['name'])) {
                ...
                redirect(base_url("agent/members/edit_member/" . urlencode(base64_encode($cId))), 'refresh');
