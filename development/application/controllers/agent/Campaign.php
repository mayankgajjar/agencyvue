...
class Campaign extends CI_Controller{
    ...
    public function index() {
        ...
        $this->template->load('agent_header', 'agent/campaign/index', $data);
...

    {
        ...
        $brokerId = $this->session->userdata('agent_primary')['broker_id'];

        $query = "SELECT * FROM crm_broker_personal WHERE broker_id = {$brokerId}";

        $query = $this->db->query($query);

        $broker = $query->row_array();

        $params = array(

            'Login' => $broker['personal_email_address'],
            ...
            'Address' => $broker['personal_address'],

            'City' => $broker['personal_city'],

            'State' => $broker['personal_state'],

            'Country' => 'United States',

            'Zip' => $broker['personal_zipcode'],

            'Phone' => substr($broker['personal_phone_number'], 0, 3).'-'.substr($broker['personal_phone_number'], 3, 3).'-'.substr($broker['personal_phone_number'],6).' Ext. 456',

            'Contact_Email' => $broker['personal_email_address'],

            'Lead_Email' => $broker['personal_email_address'],
        ...
        if($response['error'] == false){
            ...
            $query = "UPDATE crm_broker_personal SET boberdoo_user_id = {$boberdooUserId} WHERE broker_id = {$broker['broker_id']}";
