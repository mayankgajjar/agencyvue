...
class Members extends CI_Controller {
    ...
    public function index() {
        ...
        $data['title'] = 'Client Management || Admin';
    ...
    public function membersJson() {
        ...
        if ($rResult) {
            ...
            foreach ($rResult as $aRow) {
                ...
                if ($aRow['is_active'] == "Y") {
                    $row[] .= '<a class="btn btn-info btn-xs table-action-btn admin-action-icon-in-data-table" href="' . base_url() . 'admin/members/view_profile?customer_id=' . urlencode(base64_encode($aRow["customer_id"])) . '"title="View Client Profile"><i class="glyphicon glyphicon-eye-open" title="View Profile"></i></a>'
                            . '<a href="' . base_url() . 'admin/members/edit_member/' . urlencode(base64_encode($aRow["customer_id"])) . '" class="btn btn-default btn-xs table-action-btn admin-action-icon-in-data-table"><i class="fa fa-pencil" title="Edit Client"></i></a>'
                            . '<button class="danger-alert btn btn-danger btn-xs table-action-btn del_lead admin-action-icon-in-data-table remove-span" data-custom-value="' . $aRow["customer_id"] . '"><i class="glyphicon glyphicon-trash" title="Remove Client"></i></button>'
                            ...
                            . '<span class="danger-alert btn btn-danger btn-xs table-action-btn inactive-member admin-action-icon-in-data-table remove-span" data-custom-value="' . $aRow["customer_id"] . '"><i class="glyphicon glyphicon-remove" title="In-Active Client"></i></span>'
                            . '<span class="btn btn-info btn-xs table-action-btn admin-action-icon-in-data-table change-password" data-custom-value="' . $aRow["customer_id"] . '"><i class="glyphicon glyphicon-lock" title="Change Password"></i></span>';
                } else {
                    $row[] .= '<a class="btn btn-info btn-xs table-action-btn admin-action-icon-in-data-table" href="' . base_url() . 'admin/members/view_profile?customer_id=' . urlencode(base64_encode($aRow["customer_id"])) . '"title="View Client Profile"><i class="glyphicon glyphicon-eye-open" title="View Profile"></i></a>'
                            . '<a href="' . base_url() . 'admin/members/edit_member/' . urlencode(base64_encode($aRow["customer_id"])) . '" class="btn btn-default btn-xs table-action-btn admin-action-icon-in-data-table"><i class="fa fa-pencil" title="Edit Client"></i></a>'
                            . '<button class="danger-alert btn btn-danger btn-xs table-action-btn del_lead admin-action-icon-in-data-table remove-span" data-custom-value="' . $aRow["customer_id"] . '"><i class="glyphicon glyphicon-trash" title="Remove Client"></i></button>'
                            . '<button class="danger-alert btn btn-primary btn-xs table-action-btn lead_det admin-action-icon-in-data-table info-span" data-custom-value="' . $aRow["customer_id"] . '"><i class="fa fa-list-ul" title="View Log"></i></button>'
                            . '<span class="danger-alert btn btn-success btn-xs table-action-btn active-member admin-action-icon-in-data-table remove-span" data-custom-value="' . $aRow["customer_id"] . '"><i class="glyphicon glyphicon-ok " title="Active Client"></i></span>'
                            . '<span class="btn btn-info btn-xs table-action-btn admin-action-icon-in-data-table change-password" data-custom-value="' . $aRow["customer_id"] . '"><i class="glyphicon glyphicon-lock" title="Change Password"></i></span>';
                }
                $output['aaData'][] = $row;
            }
    ...
    public function edit_member($id = null) {
        ...
        if (($this->input->post('save'))) {
            ...
            echo redirect(base_url() . 'admin/members');
        }
        ...
        if (sizeof($data['buyProductID']) > 0) {
            ...
            $data['product_array01'] = '';
        }

        /* $data['product_array01'] = $this->product->findproduct($state_id, $main_age, $zip, $weights,$id_str); */
    ...
    public function inactivemember() {
        ...
        if ($done) {
            ...
            echo "Client Successfully Inactive";
        } else {
    ...
    public function activemember() {
        ...
        if ($done) {
            ...
            $res = insert_update_data($ins = 1, $tbl_name = 'crm_member_log', $log_data = $log_data);
            echo "Client Successfully Active";
        } else {
            echo "Error Into Active Client";
        }
    }
    ...
    public function removemember() {
        ...
        $done = insert_update_data($ins = 0, $tbl_name = 'crm_lead_member_master', $data = $data, $where = $con);
        // is_deleted from users table
        $gUser = get_global_userID($cus_id);
        $gWhere = array('user_id' => $gUser);
        $isDelete = array('is_deleted' => 'Y');
        $userTbl = insert_update_data($ins = 0, $tbl_name = 'crm_user_tbl', $isDelete, $gWhere);
        if ($done && $userTbl) {
            echo "Client Successfully Delete";
        }
        die();
    }
    ...
    public function change_password_by_admin() {
        ...
        $user = get_global_userID($cus_id);
        $data_personal = get_data("crm_user_tbl", 1, "user_id = $user");
