...
class Products extends CI_Controller {
    ...
    public function mp3_upload($files)
    {
        $ascript = $files;
        ...
        if ($rResult) {
            foreach ($rResult as $aRow) {
                ...
                $row[] .= '<span class="btn btn-info btn-xs table-action-btn admin-action-icon-in-data-table copy_product" data-custom-value="' . urlencode(base64_encode($aRow["global_product_id"])) . '" ><i class="fa fa-copy" title="Copy Product"></i></span>'
                        . '<a class="btn btn-default btn-xs table-action-btn admin-action-icon-in-data-table" href="' . base_url() . 'admin/products/addProduct/' . urlencode(base64_encode($aRow["global_product_id"])) . '"><i class="fa fa-pencil" title="Edit Product"></i></a>'
