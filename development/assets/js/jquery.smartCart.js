...
(function ($, window, document, undefined) {
    ...
    var defaults = {
        ...
        paramSettings: {// Map the paramters
            ...
            productQuantity: 'product_enrollment',
    ...
    $.extend(SmartCart.prototype, {
        ...
        _setElements: function () {
            ...
            this.cart_element.append('<div class="panel-heading sc-cart-heading cart-total">' + this.options.lang.cartTitle + ' <span class="sc-cart-count badge">0</span></div>');
        ...
        _setToolbar: function () {
            ...
            if (this.options.toolbarSettings.showCheckoutButton) {
                ...
                switch (this.options.toolbarSettings.checkoutButtonStyle) {
                    ...
                    case 'image':
                        ...
                        break;
                    default:
                        btnCheckout = '<button class="btn btn-info sc-cart-checkout checkout-btn" type="button">' + this.options.lang.checkout + '</button> ';
                        break;
                }
            ...
            if (this.options.toolbarSettings.showClearButton) {
                var btnClear = $('<button class="btn btn-danger sc-cart-clear clear-btn" type="button">').text(this.options.lang.clear);
            ...
            if (this.options.toolbarSettings.showCartSummary) {
                var panelSubTotal = $('<div class="sc-cart-summary-subtotal sub-total">');
        ...
        _setEvents: function () {
            ...
            $(this.options.addCartSelector).on("click", function (e) {
                ...
                $(this).siblings(":button").val(p.unique_key);
            });
            ...
            $(document).on("click", '.remove-product', function (e) {
                ...
                if (n.unique_key == key) {
                    console.log('ma');
                        ...
                        mi.cart.splice(i, 1);
                        $('*[data-product-unique-key="' + key + '"]').removeClass('sc-added-item');
                        mi._hasCartChange();
                ...
                $('div[data-unique-key="'+$(this).val()+'"]').remove();
                mi._hasCartChange();

                //this._addUpdateCartItem(e);
            });
        ...
        _addUpdateCartItem: function (p) {
            ...
            if (elmMain && elmMain.length > 0) {
                ...
                elmMain.append('<button type="button" class="sc-cart-remove product-remove-btn">' + this.options.lang.cartRemove + '</button>');
                elmMain.attr('data-unique-key', p.unique_key);
                elmMain.append(this._formatTemplate(this.options.cartItemTemplate, p));
                var fees = jQuery.parseJSON(this._getValueOrEmpty(p[this.options.paramSettings.productQuantity]));
                ...
                var itemSummary = '<div class="sc-cart-item-summary"><select class="sc-cart-item-qty enrollment-fee">';
                itemSummary += '<option value="0">Enrollment Fee </option>'
                for (i = 0; i < fees.length; i++) {
                    itemSummary += '<option value="' + fees[i] + '">$' + fees[i] + '</option>'
                    // console.log(fees[i]);
                }
                itemSummary += '</select>';
                itemSummary += '<span class="sc-cart-item-price">' + this._getMoneyFormatted(p[this.options.paramSettings.productPrice]) + '</span></div>';
        ...
        _hasCartChange: function () {
            ...
            $('.sc-cart-totalpayment', this.element).text(this._getTottalPayment());
            $('.sc-cart-enrollment-fee', this.element).text(this._getEnrollmentFees());

console.log('Mayanbkj');
        ...
        _getCartSubtotal: function () {
            ...
            $.each(this.cart, function (i, p) {
                if (mi._getValidateNumber(p[mi.options.paramSettings.productPrice])) {
                    if(p[mi.options.paramSettings.productQuantity].length >= '4'){
                        subtotal += (p[mi.options.paramSettings.productPrice] - 0) + 0;
                    }else{
         ...
         /*
         * Calculates the cart Total Payment
         */
        _getTottalPayment: function () {
            var mi = this;
            var tottalpayment = 0;

            $.each(this.cart, function (i, p) {
                if (mi._getValidateNumber(p[mi.options.paramSettings.productPrice])) {
                    tottalpayment = (p[mi.options.paramSettings.productPrice] - 0)+(tottalpayment - 0);
                }

            });
            return this._getMoneyFormatted(tottalpayment);
        },

        /*
         * Calculates the cart Total Payment
         */
        _getEnrollmentFees: function () {
            var mi = this;
            var enrollmentfee = 0;

            $.each(this.cart, function (i, p) {
                if (mi._getValidateNumber(p[mi.options.paramSettings.productQuantity])) {
                    enrollmentfee = (p[mi.options.paramSettings.productQuantity] - 0)+(enrollmentfee - 0);
                }

            });
            return this._getMoneyFormatted(enrollmentfee);
        },
        /*
